"""
Import Python modules directly from GitHub repositories with progress indication.
"""
import os
import sys
import tempfile
...
SERVICE_NAME = "github_repo_importer"
...
def _get_repo_url(repo_path: str) -> str:
    ...
    if repo_path.startswith("http"):
        base_url = repo_path
        # Extract repo path from URL if needed
        if "github.com/" in base_url:
            path_part = base_url.split("github.com/")[-1].replace(".git", "")
        else:
            path_part = repo_path
    else:
    ...
    token = get_token()
    # Try the format that GitHub recommends
    auth_url = f"https://{token}:x-oauth-basic@github.com/{path_part.rstrip('/')}"
...
def _get_local_path(repo_name: str) -> str:
    ...
    base_dir = os.path.join(tempfile.gettempdir(), "github_repo_importer")
    os.makedirs(base_dir, exist_ok=True)
    return os.path.join(base_dir, repo_name)

class GitProgressHandler(git.remote.RemoteProgress):
    """Progress handler for Git operations with tqdm integration."""
    ...
    def update(self, op_code, cur_count, max_count=None, message=''):
        if not self.pbar:
            ...
            self.pbar = tqdm(
                total=max_count if max_count else 100,
                desc=f"Downloading {self.repo_name}",
                unit="B",
                unit_scale=True,
                unit_divisor=1024,
                ascii=True  # For better compatibility
            )
        ...
        if max_count:
            ...
            # If max_count is not available, use percentage
            if message and "%" in message:
                try:
                    percentage = int(message.split("%")[0].strip())
                    self.pbar.update(percentage - self.pbar.n)
                except (ValueError, IndexError):
                    self.pbar.update(1)  # Just advance a bit
            else:
                self.pbar.update(1)  # Just advance a bit
                
        # Update description with the current operation
        if message:
            self.pbar.set_description(f"Downloading {self.repo_name}: {message}")
        
        # Close progress bar when finished
        if op_code & self.DONE:
            if self.pbar:
                self.pbar.close()

def _clone_repo(repo_path: str, branch: str = "main") -> str:
    """Clone a repository to local storage with progress indication."""
    ...
    # Clone the repository with progress handler
    try:
        progress = GitProgressHandler(repo_name)
        git.Repo.clone_from(auth_url, local_path, branch=branch, progress=progress)
        print(f"\n✅ Repository successfully cloned to {local_path}")
    except git.exc.GitCommandError as e:
        if "403" in str(e):
            raise PermissionError(f"❌ Access denied to repository {repo_path}. Check your token permissions.") from e
        elif "not found" in str(e).lower():
            raise FileNotFoundError(f"❌ Repository {repo_path} not found. Check if the repository exists and is accessible.") from e
        else:
...
def _create_module(name: str, path: str) -> types.ModuleType:
    ...
    if os.path.isfile(path) and path.endswith(".py"):
        ...
        # Add __file__ attribute if __init__.py exists
        init_path = os.path.join(path, "__init__.py")
        if os.path.exists(init_path):
            module.__file__ = init_path
        else:
            module.__file__ = path  # Just use the directory as __file__ if no __init__.py
        ...
        for item in os.listdir(path):
            ...
            if item.endswith(".py"):
                ...
                # It's a subdirectory that's not a special dir
...
def import_module_from_path(module_name: str, module_path: str) -> types.ModuleType:
    """
